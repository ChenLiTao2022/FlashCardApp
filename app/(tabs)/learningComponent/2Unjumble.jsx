...
export default function Unjumble({ dueCards, onAnswer, showResult }) {
  const [loading, setLoading] = useState(true);
  const [selectedCard, setSelectedCard] = useState(null);
  const [selectedExample, setSelectedExample] = useState(null);
  const [isUsingQuestion, setIsUsingQuestion] = useState(false);
      ...
      try {
        ...
        if (randomCard.examples) {
          ...
          if (examples && examples.length > 0) {
            ...
            setIsUsingQuestion(useQuestion);
            
            // Get the text to unjumble and split it
            let textToUnjumble = useQuestion ? randomExample.question : randomExample.answer;
            setOriginalSentence(textToUnjumble.replace(/#/g, ''));
            
            const words = splitTextByLanguage(textToUnjumble);
            ...
            const textToUnjumble = useQuestion ? randomCard.front : randomCard.back;
            setOriginalSentence(textToUnjumble.replace(/#/g, ''));
            ...
            setSelectedExample({
              ...
              answerPhonetic: '',
              translation: useQuestion ? randomCard.back : randomCard.front
            });
          }
        } else {
          // If no examples, use the card's front and back
        ...
        console.error("Error initializing Unjumble:", error);
  ...
  const checkAnswer = () => {
    ...
    const selectedText = selectedWords.map(word => word.text).join(/\s/.test(originalSentence) ? ' ' : '');
    const correctText = originalSentence;
    ...
    if (selectedCard && selectedExample) {
      const sentenceType = isUsingQuestion ? "Question" : "Answer";
      popupString += `${sentenceType}: ${correctText}\n`;
      
      if (isUsingQuestion && selectedExample.questionPhonetic) {
        popupString += `Phonetic: ${selectedExample.questionPhonetic}\n`;
      } else if (!isUsingQuestion && selectedExample.answerPhonetic) {
        popupString += `Phonetic: ${selectedExample.answerPhonetic}\n`;
      }
      
      popupString += `Your Arrangement: ${selectedText}\n`;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.questionContainer}>
        <Text style={styles.questionLabel}>Arrange the words in the correct order:</Text>
        {isUsingQuestion ? (
          <>
            {selectedExample.translation && (
              <Text style={styles.translationText}>{selectedExample.translation}</Text>
            )}
          </>
        ) : (
          <>
      ...
      {answered && (
        <View style={[
          ...
          <Text style={styles.answerText}>
            Correct answer: {originalSentence}
          </Text>
          {isUsingQuestion && selectedExample.questionPhonetic && (
            <Text style={styles.answerPhoneticText}>
              {selectedExample.questionPhonetic}
            </Text>
          )}
          {!isUsingQuestion && selectedExample.answerPhonetic && (
            <Text style={styles.answerPhoneticText}>
